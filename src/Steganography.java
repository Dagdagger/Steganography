...
public class Steganography
{
	 ...
	 */
	public boolean encode(String path, String original, String ext1, String stegan, String message, String password)
	{
		...
		image = add_text(image,message, password);
	...
	public String[] decode(String path, String name)
	{
		...
		try
		{
			...
			String pword = (new String(decode[0]));
			String message = (new String(decode[1]));
			end[0] = pword;
			end[1] = message;
			return end;
		}
	 ...
	 */
	private BufferedImage add_text(BufferedImage image, String text, String password)
	{
		//convert all items to byte arrays: image, message, message length
		byte img[]  = get_byte_data(image);
		byte msg[] = text.getBytes();
		byte pword[] = password.getBytes();
		int plength = pword.length*8;
		byte len[]   = bit_conversion(msg.length);
		byte poff[] = bit_conversion(pword.length);
		try
		{
			encode_text(img, len,  0); //0 first positioning
			encode_text(img, poff, 32);
			encode_text(img, pword, 64);
			encode_text(img, msg, 64+plength ); //4 bytes of space for length: 4bytes*8bit = 32 bits
		}
		catch(Exception e)
		{
			JOptionPane.showMessageDialog(null, 
	...
	private byte[][] decode_text(byte[] image)
	{
		...
		for(int i=32; i<64; ++i )
		{
			plength = (plength << 1) | (image[i] & 1);
		}
		
		
		byte[] pword = new byte[plength];
		
		secondoffset = pword.length*8 + 64;
		
		for (int b=0; b<pword.length; ++b )
		{
			for(int i = 0; i< 8; ++i, ++offset)
			{
				pword[b] = (byte)((pword[b] << 1) | (image[offset] & 1));
			}
		}
		...
		for(int b=0; b<result.length; ++b )
		{
			...
			for(int i=0; i<8; ++i, ++secondoffset)
			{
				//assign bit: [(new byte value) << 1] OR [(text byte) AND 1]
				result[b] = (byte)((result[b] << 1) | (image[secondoffset] & 1));
			}
		}
		
		byte end[][] = {pword,result};
